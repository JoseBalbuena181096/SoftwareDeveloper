...
const usuarios = [
  ...
  while (indexUsuario < usuarios.length) {
    ...
    while (indexPublicacion < usuario.publicaciones.length) {
      ...
      if(usuarios[indexUsuario].publicaciones[indexPublicacion].type === "alquiler" ){
        console.log(`${usuarios[indexUsuario].nombre} tiene la publicaion en alquiler: 
             ${usuarios[indexUsuario].publicaciones[indexPublicacion].title}`);
      }
